...
function generateLogs(type, player1, player2) {
  ...
  switch (type) {
    case "start":
      ...
      break;
    case "defence":
      text = `${time} - ${logs["defence"][getRandom(randomNum)]
        .replace("[playerKick]", player1.name)
        .replace("[playerDefence]", player2.name)}`;
      break;
    case "hit":
      text = `${time} - ${logs["hit"][getRandom(randomNum)]
        .replace("[playerKick]", player2.name)
        .replace("[playerDefence]", player1.name)} - ${100 - player1.hp} [${
        player1.hp
      }/100]`;
      break;
    case "end":
      text = logs["end"][getRandom(randomNum)]
